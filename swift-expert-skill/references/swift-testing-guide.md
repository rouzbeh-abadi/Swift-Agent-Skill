...
- Persistence wrappers such as `UserDefaults` stores should be tested directly with isolated test data rather than only through UI or feature flows.
...
- For `UserDefaults` wrappers, test read, write, remove, and reset behavior with a dedicated suite.
...
- [ ] Persistence wrappers with real behavior are directly tested when appropriate
